-- Adds wrap_up_published_at to trip_card_view (docs/data-model.md). Part of
-- #95 — lets Home's "Kept forever" grid card distinguish a finished trip
-- whose wrap-up has been published ("Keep forever") from one that hasn't.
--
-- Down/revert notes: `create or replace view` back to the #28 shape (drop
-- the wrap_up_published_at column and the left join).
